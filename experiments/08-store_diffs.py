    ...
    for script_path in scripts:
        ...
        for mutant_dir in os.listdir():
            ...
            for idx,d in enumerate(diff):
                diff_file.write(d,)
            diff_file.close()
            #for idx,line in enumerate(original_file_content):
            #    mut_line = mutant_file.readline()
            #    if line != mut_line:
            #        difflines.append(idx+1)
            #mutant_file.close()
            #difflines_file = open(mutant_dir+'.diff','w')
            #if len(difflines) != 1:
            #    print(">> {}.diff".format(mutant_dir))
            #for line in difflines:
            #    difflines_file.write(str(line)+'\n')
            #difflines_file.close()
            os.chdir('..')
        os.chdir('../..')
