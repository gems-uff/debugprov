...
class ProvenanceTools:
    ...
    def get_dependencies(self):
        dependencies = []
        query = ("select EV_INFLU.id as 'EV_INFLU_ID', CC_INFLU.id as 'CC_INFLU_ID', "
        "EV_DEPEND.id as 'EV_DEPEND_ID', CC_DEPEND.id as 'CC_DEPEND_ID' "
        "from dependency D "
        ...
        for tupl in self.cursor.execute(query,[]):
                dependent = Evaluation(tupl[2],tupl[3])
                influencer = Evaluation(tupl[0],tupl[1])
